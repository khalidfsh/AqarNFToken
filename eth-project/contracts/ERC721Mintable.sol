...
contract Ownable {
    ...
    modifier onlyOwner {
        require(msg.sender == _owner, "only owner of contract can call this methods");
        _;
    }

    //  4) fill out the transferOwnership function
    //  5) create an event that emits anytime ownerShip is transfered (including in the constructor)
    event ownerityTransfered(address indexed to);

    function transferOwnership(address newOwner) public onlyOwner {
        // TODO add functionality to transfer control of the contract to a newOwner.
        // make sure the new owner is a real address
        require(newOwner != address(0) && newOwner != _owner, "TODO");
        _owner = newOwner;
        emit ownerityTransfered(newOwner);
    }
...
//  4) create 'whenNotPaused' & 'paused' modifier that throws in the appropriate situation
    modifier whenNotPaused() {
     ...
     modifier paused() {
        require(_paused, "contract currently is not paused");
        _;
    }
//  5) create a Paused & Unpaused event that emits the address that triggered the event
    ...
    function setPaused(bool isPaused) external onlyOwner {
        require(_paused != isPaused, "contract paused state is allredey 'isPaused'");
        _paused = isPaused;
        if (_paused)
            emit Paused();
        else
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(to != _tokenOwner[tokenId], "can not approve same owner address");

        // TODO require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(msg.sender == _tokenOwner[tokenId] || isApprovedForAll(_tokenOwner[tokenId], msg.sender),
                "caller not owner of this token nither approved for all"
        );
...
contract AqarERC721MintableToken is ERC721Metadata {
    constructor()
        public
        ERC721Metadata(
